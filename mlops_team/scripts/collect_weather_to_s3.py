...
# import numpy as np # pd.read_csv에서 na_values 사용 시 자동 처리될 수 있음
import boto3
...
def fetch_single_hour_kma_data(target_kst_dt: datetime, station_id: str, auth_key: str):
    """
    특정 KST 시간의 특정 지점 날씨 데이터를 KMA API로부터 가져와 DataFrame으로 반환합니다.
...
def save_hourly_data_to_s3_csv(df: pd.DataFrame, observation_kst_dt: datetime, 
    ...
    """DataFrame을 시간별 파티셔닝된 CSV로 S3에 업로드합니다."""
    ...
    try:
        csv_buffer = StringIO()
        df.to_csv(csv_buffer, index=False, encoding='utf-8-sig') # Excel 호환성을 위해 utf-8-sig
